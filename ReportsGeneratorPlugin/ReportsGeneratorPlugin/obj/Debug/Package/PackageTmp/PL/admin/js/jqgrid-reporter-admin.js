...
function setCustomFormatJQGrid(d) {
    if (d.result != 'FAIL') {
        ...
        var oTest = { align: 'center', formatter: displayTestButton, sortable: false, width: 40 };
        d.colNames.push('');
        d.colModel.push(oTest);
...
function displayTestButton(cellvalue, options, rowObject) {
    var remove = "<button type='button' id='btn_test' class='btn btn-warning' onclick='JqGrid_Test(" + options.rowId + ")'><i class='fa fa-bolt'></i></button>"
    return remove;
}
...
function JqGrid_Test(rowId) {
    var sReportUUID = $("#grid0").jqGrid('getCell', rowId, 'UUID_RPT');
    $.colorbox({
        iframe: true,
        href: '/PL/admin/forms/report-test-form.aspx?sReportUUID=' + sReportUUID,
        width: "90%",
        height: "90%"
    });
}
