function getJQGridDetail() {
    SetVisibleLoading();
    sParameters = '';
    ExecReporterURLExtP(AJAX_CONTROLLER, JSON_JQGRID_STRUCT_URI + '&sNewSearch=true&bUserData=0', 'A9ADC9BFA7DB43DFC8EE54BCFB89D0BD', sParameters, 50, 1, setCustomFormatJQGrid);
}
...
function setCustomFormatJQGrid(d) {
    if (d.result != 'FAIL') {
        ...
        d.colModel[1].width = 60;
        d.colModel[2].width = 280;

        d.colNames[0] = 'ID';
        d.colNames[1] = 'NAME';
        d.colNames[2] = 'CONNECTION';
...
function JqGrid_QuoteDetail(rowId) {
    var sConnectionID = $("#grid0").jqGrid('getCell', rowId, 'ID_CONNECTION');
    $.colorbox({
        iframe: true,
        href: '/PL/admin/forms/connection-form.aspx?sConnectionID=' + sConnectionID,
        width: "50%",
        height: "90%",
        onClosed: function () {
...
function createConfiguration() {
    $.colorbox({
        iframe: true,
        href: '/PL/admin/forms/connection-form.aspx?sConnectionID=',
        width: "50%",
        height: "90%",
        onClosed: function () {
            $('#grid0').jqGrid('setGridParam', { page: 1, url: GetGrid0_URL_Reload() }).trigger("reloadGrid");
...
function GetGrid0_URL() {
    var sParameters = '';
    return getReporterURLP(AJAX_CONTROLLER, JSON_JQDATAGRID_URI, 'A9ADC9BFA7DB43DFC8EE54BCFB89D0BD', sParameters, 0, 0) + '&sNewSearch=false';
}

function GetGrid0_URL_Reload() {
    var sParameters = '';
    return getReporterURLP(AJAX_CONTROLLER, JSON_JQDATAGRID_URI, 'A9ADC9BFA7DB43DFC8EE54BCFB89D0BD', sParameters, 0, 0) + '&sNewSearch=true';
}

function GetGrid0_Export_URL() {
    var sParameters = '';
    return getReporterURLP(AJAX_CONTROLLER, JSON_EXPORT_URI, 'A9ADC9BFA7DB43DFC8EE54BCFB89D0BD', sParameters, 0, 1);
}
